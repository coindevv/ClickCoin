...
double getMoneySupply(qint64 Height)
{
    ...
    return convertCoins(pblockindex->nMoneySupply - 774840.15194814 * COIN);
}
...
{    
    if(block)
	{
        ...
        if (pindex->IsProofOfStake()) { 
            ...
            ui->moneySupplyBox->setText(QString::number(getMoneySupply(height), 'f', 6) + " CLICK");
    } 
    
    else {
        std::string txid = ui->txBox->text().toUtf8().constData();
        ui->valueBox->setText(QString::number(getTxTotalValue(txid), 'f', 6) + " CLICK"); 
        ...
        ui->feesBox->setText(QString::number(getTxFees(txid), 'f', 6) + " CLICK"); 
    }
}
